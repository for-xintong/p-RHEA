...
from baselines.pRHEA.run_pRHEA import Environment

class Runner(object):

    def __init__(self, envir, nenvs, nsteps, ngens, ninit, gamma=0.99):
        self.envir = envir
        self.nenvs = nenvs
        self.nsteps = nsteps
        self.ngens = ngens
    ...
    def evolution(self, x):
        ...
        states = state_id
        mb_obs, mb_rewards, mb_actions, mb_dones, mb_states, mb_qpos, mb_qvel = [], [], [], [], [], [], []
        ...
        for n, (rewards, dones) in enumerate(zip(mb_rewards, mb_dones)):
            rewards = rewards.tolist()
            dones = dones.tolist()
            rewards = discount_with_dones(rewards, dones, self.gamma)
            mb_rewards[n] = rewards
        return mb_obs, mb_rewards, mb_actions, mb_dones, mb_scores, mb_states, mb_qpos, mb_qvel
        ...
        for gens in range(self.ngens):
            ...
            if gens == 0:
                for i in range(self.nsteps):
                    if bestXflags[i] == 1: x[0, i*skip:(i+1)*skip] = bestXsaved[i,:]

            for i in range(NP):
                for j in range(Dim):
                    if x[i,j] < lb: x[i,j] = lb
                    if x[i,j] > ub: x[i,j] = ub
            mb_obs, mb_rewards, mb_actions, mb_dones, mb_scores, mb_states, mb_qpos, mb_qvel = self.evolution(x)
...
for game_name in ['Hopper-v2']:
    for repeat in range(1):
        main_env = gym.envs.make(game_name)
        obs = main_env.reset()
        ...
        envir = Environment()
        runner = Runner(envir, nenvs=nenvs, nsteps=nsteps, ngens=ngens, ninit=ninit)
        score = 0
        steps = 0
        scores_step = [0]
        scores_cum = [0]
        ...
        while True:
            ...
            for mb_action, mb_score, mb_done in zip(mb_actions, mb_scores, mb_dones):
                ...
                im.save(os.path.abspath('.') + "/result/%s/RHEA/frame_%d.png" % (game_name, steps))
    ...
    np.savetxt(os.path.abspath('.') + "/result/%s/RHEA/single_step_reward.csv" % game_name, np.array(scores_step))
    ...
    plt.savefig(os.path.abspath('.') + '/result/%s/RHEA/RHEA_%s.png' % (game_name, game_name), format='png')

    img_root = os.path.abspath('.') + "/result/%s/RHEA/" % game_name
    fps = 10
    fourcc = cv2.VideoWriter_fourcc(*'MJPG')
    videoWriter = cv2.VideoWriter(os.path.abspath('.') + "/result/%s/RHEA/saveVideo.avi" % game_name, fourcc, fps, img.shape[::-1][1:3])
